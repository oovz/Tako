...
export function getTerminalTimestampLabel(
...
export function getTaskStatusSummaryLabel(
  task: Pick<DownloadTaskState, "status" | "chapters">
): string {
  ...
  switch (task.status) {
    ...
    case "failed":
      return t("options_taskFailed", [
        String(completedChapters),
        String(totalChapters),
      ])
    case "canceled":
      return t("options_taskCanceled", [
        String(completedChapters),
        String(totalChapters),
      ])
    case "queued":
    ...
    case "downloading":
      return t("options_taskDownloading", [
        String(completedChapters),
        String(totalChapters),
      ])
    case "partial_success":
      return t("options_taskPartial", [
        String(completedChapters),
        String(totalChapters),
      ])
    default:
...
export function getTaskStatusBadge(status: DownloadTaskState["status"]): {
  label: string
  className: string
} {
  switch (status) {
    case "downloading":
      return {
        label: t("status_downloading"),
        className: "bg-primary text-primary-foreground",
      }
    case "queued":
      return {
        label: t("status_queued"),
...
export function getChapterImageSummary(
  chapter: Pick<TaskChapter, "status" | "totalImages" | "imagesFailed">
): string {
  const totalImages =
    typeof chapter.totalImages === "number" ? chapter.totalImages : 0
  const imagesFailed =
    typeof chapter.imagesFailed === "number" ? chapter.imagesFailed : 0
  ...
  if (chapter.status === "partial_success") {
    return t("options_imagesFailed", [
      String(imagesSucceeded),
      String(totalImages),
      String(imagesFailed),
    ])
  }

  return t("options_imagesSummary", [
    String(imagesSucceeded),
    String(totalImages),
  ])
}
...
export function formatChapterStatusLabel(
...
export function partitionDownloadTasks(
