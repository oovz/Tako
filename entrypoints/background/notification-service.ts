  ...
  private async readPersistedDownloadId(notificationId: string): Promise<number | undefined> {
    ...
    try {
      ...
      const task = queue
        ...
        .find((entry): entry is DownloadTaskState => entry !== null && entry.id === taskId);

      return typeof task?.lastSuccessfulDownloadId === 'number'
        ? task.lastSuccessfulDownloadId
        : undefined;
    } catch (error) {
      logger.debug('[NotificationService] Failed to resolve persisted notification click target', {
        notificationId,
        error,
      });
      return undefined;
