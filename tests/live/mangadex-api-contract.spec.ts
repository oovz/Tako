  ...
  try {
    ...
    for (let attempt = 0; attempt <= MANGADEX_LIVE_REQUEST_RETRIES; attempt++) {
      const result = await page.evaluate(
        async (fetchUrl: string): Promise<MangadexFetchResult> => {
...
test.describe("MangaDex API contract (live)", () => {
  test("manga endpoint returns expected core fields", async ({ context }) => {
    ...
    expect(dataObj.attributes as Record<string, unknown>).toHaveProperty(
      "title"
    )
  ...
  test("manga feed endpoint returns chapter payload shape", async ({
    ...
    if (chapters.length > 0) {
      ...
      expect(chapter.attributes as Record<string, unknown>).toHaveProperty(
        "translatedLanguage"
      )
    }
  })
  ...
  test("at-home endpoint returns server data or rate-limit headers", async ({
    ...
    if (!resolvedChapterId) {
      ...
      if (feedResult.ok) {
        ...
        const feedChapters = Array.isArray(feedData.data)
          ? (feedData.data as Array<Record<string, unknown>>)
          : []
        if (
          feedChapters.length > 0 &&
          typeof feedChapters[0]?.id === "string"
        ) {
