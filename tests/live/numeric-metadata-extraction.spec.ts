...
test.describe("Live numeric metadata extraction", () => {
  ...
  test("extracts chapter numbers and preserves absent volume numbers from live Shonen Jump+ state", async ({
    ...
    expect(parsedChapters.length).toBeGreaterThan(0)
    // Shonen Jump+ currently serves the series thumbnail either directly from
    // its image CDN or through its Gigaviewer image-scaling proxy. Keep this
    // as a narrow source canary without assuming one immutable CDN hostname.
