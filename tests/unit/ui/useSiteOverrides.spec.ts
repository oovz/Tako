...
describe("__shouldReloadSiteOverridesForTests", () => {
  it("returns true when local area has new value for the storage key", () => {
    const changes = {
      [SITE_OVERRIDES_STORAGE_KEY]: {
        newValue: { mangadex: { outputFormat: "cbz" } },
      },
    }
  ...
  it("respects custom storage key parameter", () => {
    ...
    expect(
      __shouldReloadSiteOverridesForTests(changes, "local", customKey)
    ).toBe(true)
    expect(
      __shouldReloadSiteOverridesForTests(changes, "local", "wrongKey")
    ).toBe(false)
  })
