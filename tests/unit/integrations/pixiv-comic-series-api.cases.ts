  ...
  describe("Pixiv Comic integration", () => {
        ...
        json: async () => ({
          data: {
            official_work: {
              ...
              image: {
                main_big:
                  "https://img-comic.pximg.net/images/work_main/9012.jpg",
                thumbnail:
                  "https://public-img-comic.pximg.net/images/work_thumbnail/9012.jpg",
              },
            },
          },
        }),
    ...
    it("does not infer Pixiv volume metadata from chapter title text without an explicit API volume field", async () => {
      ...
      const chapters = Array.isArray(chapterResult)
        ? chapterResult
        : chapterResult.chapters
      const volumes = Array.isArray(chapterResult)
        ? []
        : (chapterResult.volumes ?? [])
    ...
    it("requests chapter list using ascending order to match 最初から order", async () => {
      ...
      expect(
        calls.some((url) =>
          url.includes("/api/app/works/9012/episodes/v2?order=asc")
        )
      ).toBe(true)
    })
