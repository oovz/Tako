...
): Pick<DownloadTaskState, "status" | "chapters"> {
  return {
    status,
    chapters,
  }
...
describe("DownloadsTab status summary behavior", () => {
  ...
  it("renders queued/downloading/canceled summaries with expected wording", () => {
    ...
    expect(getTaskStatusSummaryLabel(makeTask("queued", chapters))).toBe(
      "3 chapters queued"
    )
    expect(getTaskStatusSummaryLabel(makeTask("downloading", chapters))).toBe(
...
describe("DownloadsTab chapter status badge behavior", () => {
  it("maps chapter statuses to expected badge classes", () => {
    expect(chapterStatusBadgeClass("completed")).toBe(
      "bg-primary/10 text-primary"
    )
    expect(chapterStatusBadgeClass("partial_success")).toBe(
      "bg-amber-500/20 text-amber-700"
    )
    expect(chapterStatusBadgeClass("failed")).toBe(
      "bg-destructive text-destructive-foreground"
    )
    expect(chapterStatusBadgeClass("downloading")).toBe(
      "bg-primary text-primary-foreground"
    )
    expect(chapterStatusBadgeClass("queued")).toBe(
      "bg-muted text-muted-foreground"
    )
  })

  it("falls back to muted style for unknown chapter statuses", () => {
    expect(chapterStatusBadgeClass("unknown-status")).toBe(
      "bg-muted text-muted-foreground"
    )
  })
})
