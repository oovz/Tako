...
describe("useChapterSelections helpers", () => {
  ...
  it("stores chapter selections per series independently", () => {
    let selections = {}
    selections = setSelectionsForSeries(selections, "mangadex#series-1", [
      "a",
      "b",
    ])
    selections = setSelectionsForSeries(selections, "mangadex#series-2", ["x"])

    expect(getSelectionsForSeries(selections, "mangadex#series-1")).toEqual([
      "a",
      "b",
    ])
    expect(getSelectionsForSeries(selections, "mangadex#series-2")).toEqual([
      "x",
    ])
  })

  it("deduplicates and preserves insertion order for a series selection set", () => {
    const selections = setSelectionsForSeries({}, "mangadex#series-1", [
      "a",
      "b",
      "a",
      "c",
