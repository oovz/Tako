...
function makeChapter(
  partial: Partial<ChapterState> & { url: string; title: string }
): ChapterState {
  return {
...
describe("groupChapters (Side Panel)", () => {
  it("preserves mixed standalone/volume order while grouping chapters into volumes", () => {
    ...
    if ("chapters" in grouped[1]) {
      ...
      expect(grouped[1].chapters.map((ch) => ch.url)).toEqual([
        "v2-c2",
        "v2-c1",
      ])
    }
    ...
    if ("chapters" in grouped[3]) {
      ...
      expect(grouped[3].chapters.map((ch) => ch.url)).toEqual([
        "v1-c4",
        "v1-c3",
      ])
    }
  ...
  it("preserves previous collapsed state per volume number", () => {
    ...
    const v1 = grouped.find(
      (item) => "chapters" in item && item.number === 1
    ) as any
    const v2 = grouped.find(
      (item) => "chapters" in item && item.number === 2
    ) as any
  ...
  it("groups explicit volumes by volumeId instead of synthetic volume numbers", () => {
    ...
    expect(
      grouped.map((item) => ("chapters" in item ? item.groupId : "standalone"))
    ).toEqual(["single-issues", "extras", "chapters"])
    expect(
      grouped.map((item) => ("chapters" in item ? item.title : item.title))
    ).toEqual(["单行本", "番外篇", "单话"])
  })
  ...
  it("preserves canonical chapter ids when mapping MangaPageState chapters into sidepanel rows", () => {
    ...
    const flattened = grouped.flatMap((item) =>
      "chapters" in item ? item.chapters : [item]
    )
