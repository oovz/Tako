...
describe("site-integration-initialization singleton pattern", () => {
  ...
  beforeEach(async () => {
    ...
    registerSiteIntegrationMock =
      registryModule.registerSiteIntegration as ReturnType<typeof vi.fn>
  })
  ...
  it("normalizes malformed enablement storage changes through the shared parser", async () => {
    ...
    const onChangedListener = storageOnChangedAddListener.mock
      .calls[0]?.[0] as (
...
describe("site-integration-initialization full integration lookup", () => {
  ...
  it("upgrades metadata-only registrations when a background integration is requested", async () => {
    ...
    const { getBackgroundSiteAdapterById } =
      await import("@/src/runtime/background-site-integration-initialization")

    const integration = await getBackgroundSiteAdapterById("pixiv-comic")

    expect(integration?.background.prepareDispatchContext).toEqual(
      expect.any(Function)
    )
    ...
    const backgroundRegistration = registerSiteIntegration.mock.calls
      .map(([info]) => info)
      .find(
        (info: { id?: string; integration?: unknown }) =>
          info.id === "pixiv-comic" && info.integration
      )
    ...
    expect(
      (backgroundRegistration as { integration?: { content?: unknown } })
        .integration?.content
    ).toBeUndefined()
  })
  ...
  it("registers offscreen chapter runtimes separately from service-worker runtimes", async () => {
    ...
    const registry = new Map<string, unknown>()
    const registerSiteIntegration = vi.fn(
      (info: { id: string; integration?: unknown }) => {
        const existing = registry.get(info.id) as
          { integration?: Record<string, unknown> } | undefined
        registry.set(
          info.id,
          existing && info.integration
            ? {
                ...existing,
                ...info,
                integration: {
    ...
    expect(pixivRegistration.integration?.offscreen?.chapter).toEqual(
      expect.any(Object)
    )
  ...
  it("all enabled manifests stay metadata-only", async () => {
    const { SITE_INTEGRATION_MANIFESTS } =
      await import("@/src/site-integrations/manifest")
