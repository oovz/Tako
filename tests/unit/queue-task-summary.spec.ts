...
): DownloadTaskState["settingsSnapshot"] {
...
function makeTask(
  ...
  return {
    ...
    settingsSnapshot:
      ...
      makeSettingsSnapshot(overrides.siteIntegrationId ?? siteIntegrationId),
...
describe("toQueueTaskSummary", () => {
  ...
  it("carries completed timestamp and structured failure category only", () => {
    const task = makeTask({
      status: "failed",
      started: 100,
      completed: 200,
      errorMessage: "https://signed.example/image?token=secret",
    ...
    expect(summary.failureCategory).toBe("provider_changed")
  ...
  it("never infers a presentation category by parsing technical strings", () => {
    const otherError = makeTask({
      status: "failed",
      errorMessage: "ERR_FILE_ACCESS_DENIED https://example.com/?token=secret",
    })

    const otherSummary = toQueueTaskSummary(otherError)

    expect(otherSummary.failureCategory).toBe("unknown")
    expect(toRecord(otherSummary)).not.toHaveProperty("failureReason")
  })
