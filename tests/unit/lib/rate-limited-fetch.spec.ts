...
describe("rate-limited-fetch bridge", () => {
  it("delegates to core rate limiter with the same arguments", async () => {
    ...
    const coreRateLimitedFetch = vi.mocked(
      coreModule.rateLimitedFetchByUrlScope
    )
    ...
    const result = await rateLimitedFetchByUrlScope(
      "https://example.com/image.jpg",
      "image",
      init
    )

    expect(coreRateLimitedFetch).toHaveBeenCalledWith(
      "https://example.com/image.jpg",
      "image",
      init
    )
