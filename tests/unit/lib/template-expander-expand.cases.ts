  ...
  describe("Template Expander", () => {
    ...
    describe("expandTemplate - Basic Macro Expansion", () => {
      it("expands all macros correctly", () => {
        const template =
          "<INTEGRATION_NAME>/<PUBLISHER>/<SERIES_TITLE>/<CHAPTER_TITLE>/<VOLUME_TITLE>/<CHAPTER_NUMBER_PAD2>/<CHAPTER_NUMBER_PAD3>/<VOLUME_NUMBER_PAD2>/<YYYY>/<MM>/<DD>"
    ...
    describe("expandTemplate - Error Handling", () => {
      it("errors when critical SERIES_TITLE macro is unavailable", () => {
        ...
        expect(result.errors).toContain(
          "Required macros unavailable: <SERIES_TITLE>"
        )
      })
      ...
      it("errors when critical CHAPTER_TITLE macro is unavailable", () => {
        ...
        expect(result.success).toBe(false)
        expect(result.errors).toContain(
          "Required macros unavailable: <CHAPTER_TITLE>"
        )
      })
      ...
      it("warns when optional PUBLISHER macro is unavailable", () => {
        ...
        expect(result.warnings).toContain(
          "Optional macros unavailable: <PUBLISHER>"
        )
      })
      ...
      it("errors on unknown macros", () => {
        ...
        expect(result.success).toBe(false)
        expect(result.errors).toContain(
          "Unknown macros in template: <UNKNOWN_MACRO>"
        )
      })
    ...
    describe("expandTemplate - Character Sanitization", () => {
      ...
      it("removes control characters", () => {
        ...
        expect(result.success).toBe(true)
        // sanitizeFilename replaces control chars with '_' (per spec delegation)
        expect(result.expanded).toBe("Series_With_Control")
      })
