...
import {
  ...
  baseEdgeContext,
} from "./template-resolver-test-setup"
  ...
  describe("template-resolver", () => {
    describe("resolveDownloadDirectory", () => {
      it("resolves simple template with series title", () => {
        const result = resolveDownloadDirectory(
          "<SERIES_TITLE>",
          baseDirectoryContext
        )
      ...
      it("resolves template with chapter title", () => {
        ...
        expect(result.resolvedPath).toBe(
          "One Piece/Chapter 1001 - Big Moms Rage"
        )
      })
      ...
      it("resolves template with chapter number (padded)", () => {
        const result = resolveDownloadDirectory(
          "<SERIES_TITLE>/Ch<CHAPTER_NUMBER_PAD3>",
          baseDirectoryContext
        )
        expect(result.success).toBe(true)
      ...
      it("sanitizes unsafe characters in resolved path", () => {
        const ctx = {
          ...baseDirectoryContext,
          seriesTitle: "Series: Title / With * Invalid",
        }
      ...
      it("treats extension-like suffix as directory name", () => {
        const result = resolveDownloadDirectory(
          "<SERIES_TITLE>/<CHAPTER_TITLE>.cbz",
          baseDirectoryContext
        )
        expect(result.success).toBe(true)
      ...
      it("resolves complex multi-level template", () => {
        const template =
          "<YYYY>/<PUBLISHER>/<SERIES_TITLE>/Vol<VOLUME_NUMBER_PAD2>/<CHAPTER_TITLE>"
        const result = resolveDownloadDirectory(template, baseDirectoryContext)
        expect(result.success).toBe(true)
        expect(result.resolvedPath).toBe(
          "2024/Weekly Shonen Jump/One Piece/Vol99/Chapter 1001 - Big Moms Rage"
        )
      })
    ...
    describe("edge cases and boundary conditions", () => {
      ...
      it("handles special characters that need sanitization", () => {
        const ctx = {
          ...baseEdgeContext,
          chapterTitle: 'Ch:1 <Part> "2" | Final?',
        }
