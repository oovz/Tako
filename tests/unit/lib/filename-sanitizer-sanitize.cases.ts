  ...
  describe("sanitizeFilename", () => {
    describe("Basic sanitization", () => {
      it("returns empty string for non-string input", () => {
        // `as any` is intentional: testing runtime defense against corrupted-storage
        // values that bypass TypeScript's compile-time string-only signature.
      ...
      it("keeps valid special characters", () => {
        expect(sanitizeFilename("file-name_with.valid")).toBe(
          "file-name_with.valid"
        )
    ...
    describe("Illegal character replacement", () => {
      ...
      it("replaces all illegal characters in one string", () => {
        expect(sanitizeFilename('illegal:/\\?<>*|"chars')).toBe(
          "illegal_________chars"
        )
      })
    })
    ...
    describe("Edge cases and combinations", () => {
      ...
      it("sanitizes realistic manga filenames", () => {
        expect(sanitizeFilename("Chapter 5: The Final Battle?!")).toBe(
          "Chapter 5_ The Final Battle_!"
        )
        expect(sanitizeFilename('Vol.3 Ch.12 - "Victory"')).toBe(
          "Vol.3 Ch.12 - _Victory_"
        )
        expect(sanitizeFilename("Attack on Titan - Ch 139 [END]")).toBe(
          "Attack on Titan - Ch 139 [END]"
        )
      })
    ...
    describe("Special manga-related edge cases", () => {
      it("sanitizes chapter titles with common problematic patterns", () => {
        expect(sanitizeFilename('Ch.1: "New Beginning"')).toBe(
          "Ch.1_ _New Beginning_"
        )
        expect(sanitizeFilename("Chapter 5 - Part 1/2")).toBe(
          "Chapter 5 - Part 1_2"
        )
        expect(sanitizeFilename("Vol.2 Ch.10 <Final>")).toBe(
          "Vol.2 Ch.10 _Final_"
        )
      })

      it("preserves Japanese and Chinese characters in manga titles", () => {
        expect(sanitizeFilename("第1話：新しい始まり")).toBe(
          "第1話：新しい始まり"
        )
