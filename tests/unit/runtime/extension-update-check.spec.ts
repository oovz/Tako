...
type UpdateCheckStatus = "no_update" | "update_available" | "throttled"
...
function makeRuntime(
  overrides: Partial<ExtensionUpdateRuntime> = {}
): ExtensionUpdateRuntime {
  return {
...
describe("checkForChromeWebStoreUpdate", () => {
  it("reports no_update with the installed extension version", async () => {
    ...
    await expect(
      checkForChromeWebStoreUpdate({ runtime, now: () => 1234 })
    ).resolves.toEqual({
      ok: true,
      status: "no_update",
  ...
  it("reports update_available with the Chrome Web Store version", async () => {
    const runtime = makeRuntime({
      requestUpdateCheck: vi.fn(async () =>
        updateResult("update_available", "1.2.8")
      ),
    })

    await expect(
      checkForChromeWebStoreUpdate({ runtime, now: () => 5678 })
    ).resolves.toEqual({
      ok: true,
  ...
  it("returns unsupported when the browser runtime does not expose requestUpdateCheck", async () => {
    ...
    await expect(
      checkForChromeWebStoreUpdate({ runtime, now: () => 1213 })
    ).resolves.toEqual({
      ok: false,
      status: "unsupported",
      currentVersion: "1.2.7",
  ...
  it("returns an error result when Chrome rejects the update check", async () => {
    ...
    await expect(
      checkForChromeWebStoreUpdate({ runtime, now: () => 1415 })
    ).resolves.toEqual({
      ok: false,
      status: "error",
      currentVersion: "1.2.7",
...
describe("getChromeWebStoreUpdateStatusCopy", () => {
  it("formats user-facing copy for each update-check outcome", () => {
    expect(
      ...
      description:
        "Chrome did not return an installable update for version 1.2.7. This does not compare unpacked or locally modified builds against the public Web Store listing.",
    })
    ...
    expect(
      ...
      title: "Update Ready",
      description:
        "Version 1.2.8 has been downloaded. Apply it when current extension work can be interrupted.",
    })
