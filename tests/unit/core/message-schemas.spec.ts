...
describe("message-schemas", () => {
  it("accepts GET_SETTINGS runtime command messages", () => {
    expect(ActionMessageSchema.parse({ type: "GET_SETTINGS" }).type).toBe(
      "GET_SETTINGS"
    )
  })
  ...
  it("accepts a strictly shaped MangaDex preference snapshot on MangaDex series requests", () => {
    const parsed = ActionMessageSchema.safeParse({
  ...
  it("rejects MangaDex preferences on another provider request", () => {
    const parsed = ActionMessageSchema.safeParse({
      type: "FETCH_SERIES_DATA",
      payload: {
  ...
  it("rejects undeclared STATE_ACTION enum values", () => {
    const parsed = ActionMessageSchema.safeParse({
      type: "STATE_ACTION",
      ...commandEnvelope,
      action: 999,
  ...
  it("rejects the removed automatic FSA fallback signal", () => {
  ...
  it("accepts OFFSCREEN_STATUS and OFFSCREEN_CONTROL contract shapes", () => {
    expect(
      OffscreenMessageSchema.parse({ type: "OFFSCREEN_STATUS" }).type
    ).toBe("OFFSCREEN_STATUS")
  ...
  it("rejects incomplete OFFSCREEN_DOWNLOAD_CHAPTER settings", () => {
    const parsed = OffscreenMessageSchema.safeParse({
      type: "OFFSCREEN_DOWNLOAD_CHAPTER",
      payload: {
        jobId: "job-1",
        ...
        taskId: "task-1",
        seriesKey: "mangadex:series-1",
        book: { siteIntegrationId: "mangadex", seriesTitle: "Series" },
        chapter: {
          id: "ch-1",
          title: "Chapter 1",
          url: "https://example.com/ch/1",
        ...
        settingsSnapshot: { archiveFormat: "cbz" },
        saveMode: "downloads-api",
      },
    })
...
describe("FetchSeriesDataResponseSchema", () => {
  it("accepts a successful response with seriesMetadata and chapterList", () => {
    ...
    if (parsed.success && parsed.data.success) {
      expect(parsed.data.seriesMetadata).toEqual({
        title: "Series",
        author: "Author",
      })
    }
  ...
  it("rejects a response missing the success field", () => {
    expect(
      FetchSeriesDataResponseSchema.safeParse({
        seriesMetadata: { title: "X" },
      }).success
    ).toBe(false)
  })
  ...
  it("rejects a response with a non-string error", () => {
    expect(
      FetchSeriesDataResponseSchema.safeParse({ success: false, error: 42 })
        .success
    ).toBe(false)
  })
...
describe("strict object enforcement (defense-in-depth)", () => {
  it("rejects unknown keys on START_DOWNLOAD payload", () => {
    const parsed = ActionMessageSchema.safeParse({
      type: "START_DOWNLOAD",
      ...commandEnvelope,
      payload: {
  ...
  it("rejects unknown keys on RETRY_FAILED_CHAPTERS payload", () => {
    const parsed = ActionMessageSchema.safeParse({
      type: "RETRY_FAILED_CHAPTERS",
      ...commandEnvelope,
      payload: { taskId: "task-1", extra: true },
    })
    expect(parsed.success).toBe(false)
  })

  it("rejects unknown top-level keys on STATE_ACTION messages", () => {
    const parsed = ActionMessageSchema.safeParse({
      type: "STATE_ACTION",
      ...commandEnvelope,
      action: StateAction.CLEAR_TAB_STATE,
  ...
  it("rejects unknown keys on OFFSCREEN_DOWNLOAD_CHAPTER payload", () => {
    const parsed = OffscreenMessageSchema.safeParse({
      type: "OFFSCREEN_DOWNLOAD_CHAPTER",
      payload: {
        taskId: "task-1",
  ...
  it("rejects unknown keys on OFFSCREEN_DOWNLOAD_API_REQUEST payload", () => {
    const parsed = OffscreenMessageSchema.safeParse({
