...
describe("Intelligent Progress Calculator", () => {
  ...
  describe("Overall Progress API", () => {
    ...
    it("returns finalization progress for finalizing phase", () => {
      const progress = calculator.getOverallProgress("finalizing", {
        step: "cleanup",
      })
  ...
  describe("Smooth Progress Transitions", () => {
    it("smooths progress transition with default smoothing factor", () => {
      ...
      const smoothed = calculator.calculateSmoothProgress(
        currentProgress,
        targetProgress
      )
    ...
    it("smooths progress with custom smoothing factor", () => {
      ...
      const smoothed1 = calculator.calculateSmoothProgress(
        currentProgress,
        targetProgress,
        0.1
      )
      const smoothed2 = calculator.calculateSmoothProgress(
        currentProgress,
        targetProgress,
        0.5
      )
  ...
  describe("Progress Consistency", () => {
    ...
    it("ensures startup -> download -> finalization progression", () => {
      ...
      const finalization =
        calculator.calculateFinalizationProgress("organizing")
  ...
  describe("Real-World Scenarios", () => {
    ...
    it("simulates complete download lifecycle", () => {
      ...
      for (const item of lifecycle) {
        const progress = calculator.getOverallProgress(item.phase, {
          step: item.step,
        })
        if ("expected" in item) {
