...
describe("getProgressMessage", () => {
  ...
  describe("startup phase messages", () => {
    it("returns initializing message", () => {
      expect(
        calculator.getProgressMessage("startup", { step: "initializing" })
      ).toBe("Initializing download...")
    })

    it("returns connecting message", () => {
      expect(
        calculator.getProgressMessage("startup", { step: "connecting" })
      ).toBe("Connecting to server...")
    })

    it("returns starting message for ready step", () => {
      expect(calculator.getProgressMessage("startup", { step: "ready" })).toBe(
        "Starting download..."
      )
    })

    it("falls through to ready message when step is undefined", () => {
      expect(calculator.getProgressMessage("startup", {})).toBe(
        "Starting download..."
      )
    })
  })
  ...
  describe("finalizing phase messages", () => {
    it("returns organizing message", () => {
      expect(
        calculator.getProgressMessage("finalizing", { step: "organizing" })
      ).toBe("Organizing files...")
    })

    it("returns cleanup message", () => {
      expect(
        calculator.getProgressMessage("finalizing", { step: "cleanup" })
      ).toBe("Cleaning up...")
    })

    it("returns complete message", () => {
      expect(
        calculator.getProgressMessage("finalizing", { step: "complete" })
      ).toBe("Download complete!")
    })

    it("falls through to complete message when step is undefined", () => {
      expect(calculator.getProgressMessage("finalizing", {})).toBe(
        "Download complete!"
      )
    })
  })
  ...
  it("returns generic processing message for unknown phase", () => {
    expect(calculator.getProgressMessage("unknown" as never, {})).toBe(
      "Processing..."
    )
  })
})
