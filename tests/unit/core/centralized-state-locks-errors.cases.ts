...
export function registerCentralizedStateLockAndErrorCases(): void {
  describe("Lock Management", () => {
    ...
    it("serializes concurrent lock acquisition for the same key", async () => {
      ...
      const lock2Promise = acquireLock("global_state_mutation").then(
        (release: () => void) => {
    ...
    it("times out when a lock is held too long", async () => {
      ...
      try {
        ...
        const rejection = expect(waitingLock).rejects.toThrow(
          "Lock timeout: global_state_mutation"
        )
    ...
    it("does not let a later waiter bypass a live predecessor after an earlier waiter times out", async () => {
      vi.useFakeTimers()
      ...
      try {
        ...
        const releaseFirst = await acquireLock("global_state_mutation")
        const timedOutWaiter = acquireLock("global_state_mutation", 100)
        const rejection = expect(timedOutWaiter).rejects.toThrow(
          "Lock timeout: global_state_mutation"
        )
        await vi.advanceTimersByTimeAsync(100)
        await rejection
        ...
        const thirdWaiter = acquireLock("global_state_mutation", 1000).then(
          (release) => {
            thirdAcquired = true
            return release
          }
  ...
  describe("Error Handling", () => {
    ...
    it("handles initialization failure gracefully", async () => {
      ...
      vi.mocked(chrome.storage.session.setAccessLevel).mockRejectedValueOnce(
