...
export function registerSettingsServiceUpdatesAndHelpersCases(): void {
  describe("Partial Update Deep Merge", () => {
    ...
    it.each(["system", "reduce"] as const)(
      "should update the %s motion preference without disturbing other settings",
      async (motionPreference) => {
        ...
        const settings = await settingsService.getSettings()
        expect(settings.motionPreference).toBe(motionPreference)
