...
function expectMatchedUrl(
  url: string,
  expected: { integrationId: string; role?: string }
) {
...
describe("URL Pattern Matching", () => {
  ...
  describe("Basic Pattern Matching", () => {
    ...
    it("does not advertise Pixiv chapter routes as resolvable series pages", () => {
      expect(
        matchUrl("https://comic.pixiv.net/viewer/stories/44495")
      ).toBeNull()
      expect(matchUrl("https://comic.pixiv.net/episodes/9999")).toBeNull()
    })
    ...
    it("does not treat Shonen Jump+ catalog routes as series contexts", () => {
      expect(matchUrl("https://shonenjumpplus.com/")).toBeNull()
      expect(matchUrl("https://shonenjumpplus.com/series")).toBeNull()
      expect(matchUrl("https://shonenjumpplus.com/series/4401")).toBeNull()
    })
    ...
    it("can validate a declared URL shape without enabling the integration", () => {
      setUserSiteIntegrationEnablement({ mangadex: false })

      expect(
        matchUrl("https://mangadex.org/title/abc123", {
          includeDisabled: true,
        })
      ).toMatchObject({ integrationId: "mangadex", role: "series" })
    })
  })
  ...
  describe("Exclude Pattern Enforcement", () => {
    ...
    it("excludes manhuagui chapter viewer pages", () => {
      expect(
        matchUrl("https://www.manhuagui.com/comic/28004/760110.html")
      ).toBeNull()
      expect(
        matchUrl("https://www.manhuagui.com/comic/28004/760110_p7.html")
      ).toBeNull()
    })
    ...
    it("allows title paths with UUID and slug (mangadex series pages)", () => {
      expectMatchedUrl(
