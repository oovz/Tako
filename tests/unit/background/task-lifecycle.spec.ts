...
} from "@/entrypoints/background/task-lifecycle"
...
describe("task lifecycle helpers", () => {
  ...
  it("leaves already-terminal chapters unchanged", () => {
    ...
    expect(
      normalizeInterruptedChapter(chapter, "Download interrupted", 999)
    ).toEqual(chapter)
  })
  ...
  it("normalizes interrupted tasks to partial_success when any chapters already completed", () => {
    ...
    expect(normalized.chapters.map((chapter) => chapter.status)).toEqual([
      "completed",
      "failed",
      "failed",
    ])
  ...
  it("normalizes fully interrupted tasks to failed and preserves an existing completed timestamp", () => {
    ...
    const normalized = normalizeInterruptedTask(
      task,
      "Extension updated during download",
      999
    )
    ...
    expect(
      normalized.chapters.every((chapter) => chapter.status === "failed")
    ).toBe(true)
  })
})
