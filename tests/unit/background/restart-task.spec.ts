...
describe("restartTask", () => {
  ...
  it("creates a new queued task with all chapters reset and marks original task as retried", async () => {
    ...
    const createdTask = (
      ...
      (candidate) => candidate.id === result.newTaskId
    ...
    expect(
      (await stateManager.getGlobalState()).downloadQueue.find(
  ...
  it("allows restart for failed and canceled tasks", async () => {
    ...
    expect((await stateManager.getGlobalState()).downloadQueue).toHaveLength(4)
  })
})
