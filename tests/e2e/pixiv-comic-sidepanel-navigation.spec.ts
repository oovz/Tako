...
test.describe("Pixiv Comic side panel navigation workflows (mocked)", () => {
  test("front page -> work page initializes tab state", async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test("work -> front page -> different work reinitializes tab state", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await waitForTabSeriesTitle(
      context,
      tabId,
      PixivComic.MINIMAL_SERIES.series.seriesTitle
    )
