...
test.describe("Queue actions and cross-window behavior", () => {
  ...
  test("hybrid settings: no advanced config in Side Panel and Options link opens Options page", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await expect(
      sp.getByRole("button", { name: /CBZ|ZIP|No Archive/i })
    ).toHaveCount(0)
  ...
  test("multi-window: queue updates are reflected across windows", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await sp1.goto(`chrome-extension://${extensionId}/sidepanel.html`, {
      waitUntil: "domcontentloaded",
    })

    const sp2 = await context.newPage()
    await sp2.goto(`chrome-extension://${extensionId}/sidepanel.html`, {
      waitUntil: "domcontentloaded",
    })
    ...
    await waitForGlobalState(context, (state) => {
      const t = state.downloadQueue.find(
        (task) => task.id === "multi-window-active"
      )
  ...
  test("single active task is visible consistently across tabs", async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test.describe("unlimited queue behavior", () => {
    test("allows more than 5 tasks in queue without capacity error", async ({
      context,
      extensionId,
      page,
    }) => {
      ...
      const tasks: DownloadTaskState[] = Array.from({ length: 10 }, (_, i) =>
        makeTask({
          ...
          chapters: [
            makeChapter(
              buildExampleUrl(`/unlimited-${i}`),
              i === 0 ? "downloading" : "queued"
            ),
          ],
        })
      )
      ...
      const state = await waitForGlobalState(context, (globalState) => {
        ...
        const failedCapacityTask = globalState.downloadQueue.find(
          (task) =>
            task.status === "failed" &&
            task.errorMessage?.toLowerCase().includes("capacity")
        )
      ...
      await expect(
        sp.getByRole("heading", { name: "Series 1", exact: true })
  ...
  test.describe("open-folder action visibility in side panel", () => {
    test("Side Panel does not render Open Folder action for any task state", async ({
      context,
      extensionId,
      page,
    }) => {
      ...
      await expect(
        sp.getByRole("button", { name: /open folder/i })
      ).toHaveCount(0)
  ...
  test.describe("failed-task error presentation", () => {
    test("shows safe generic error copy instead of raw lifecycle diagnostics", async ({
      context,
      extensionId,
      page,
    }) => {
      ...
      await expect(sp.locator("#root")).toBeVisible()
      await expect(sp.getByText("Update Failed Series")).toBeVisible({
        timeout: 10000,
      })
      await expect(
        sp.getByText(
      ...
      await expect(
        sp.getByText(/Extension updated during download/i)
      ).toHaveCount(0)
