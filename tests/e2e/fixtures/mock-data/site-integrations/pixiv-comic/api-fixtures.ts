...
function coverUrlFor(seriesId: string, explicit?: string): string {
  return (
    explicit ||
    `https://img-comic.pximg.net/works/${seriesId}/cover_main_big.jpg`
  )
}
 ...
 */
export function buildPixivWorkV5Response(
  workId: string
): Record<string, unknown> {
  const fixture =
    workId === BASIC_SERIES.series.seriesId
      ? BASIC_SERIES.series
  ...
  return {
    data: {
      official_work: {
        ...
        description:
          typeof fixture.description === "string" ? fixture.description : "",
        image: {
          main: coverUrlFor(
            workId,
            typeof fixture.coverUrl === "string" ? fixture.coverUrl : undefined
          ),
          main_big: coverUrlFor(
            workId,
            typeof fixture.coverUrl === "string" ? fixture.coverUrl : undefined
          ),
          thumbnail: coverUrlFor(
            workId,
            typeof fixture.coverUrl === "string" ? fixture.coverUrl : undefined
          ),
        },
 ...
 */
export function buildPixivEpisodesV2Response(
  workId: string
): Record<string, unknown> {
  const dataset =
    workId === BASIC_SERIES.series.seriesId
  ...
  const episodes = dataset.map((chapter, index) => ({
    ...
    episode: {
      ...
      sub_title: stripNumberingTitle(
        chapter.title,
        `第${chapter.chapterNumber ?? index + 1}話`
      ),
