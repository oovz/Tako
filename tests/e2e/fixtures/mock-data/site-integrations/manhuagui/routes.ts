...
export const registerManhuaguiRoutes: RouteRegistrar = async (
  context,
  options
) => {
  if (!options.useMocks) {
  ...
  await context.route(
    ...
    async (route: Route) => {
      ...
      // expected layout.
      const chapterMatch = url.pathname.match(
        /^\/comic\/(\d+)\/(\d+)(?:_p\d+)?\.html$/
      )
      if (chapterMatch) {
      ...
      if (seriesMatch) {
        ...
        if (seriesId === CATEGORY_SERIES.series.seriesId)
          return html(CATEGORY_SERIES_PAGE_HTML)
        if (seriesId === KIMETSU_SERIES.series.seriesId)
          return html(KIMETSU_SERIES_PAGE_HTML)
  ...
  await context.route(
    `https://${MANHUAGUI_CONFIG_SCRIPT_DOMAIN}/**`,
    async (route: Route) => {
      const url = new URL(route.request().url())
      if (
        url.pathname === MANHUAGUI_CONFIG_SCRIPT_PATH ||
        /^\/scripts\/config_[^/]+\.js$/.test(url.pathname)
      ) {
  ...
  await context.route(
    `https://${MANHUAGUI_MOCK_IMAGE_DOMAIN}/**`,
    async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: SMALL_PNG_MIME_TYPE,
