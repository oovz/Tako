...
test.describe("Navigation state management", () => {
  test("clears state when navigating from a supported page to an unsupported page", async ({
    ...
    const tabId = await seedTabContext(
      ...
      context,
      {
        ...
        chapters: [
          {
            id: "chapter-1",
            url: buildMangadexUrl("/chapter/ch1"),
            title: "Chapter 1",
          },
          {
            id: "chapter-2",
            url: buildMangadexUrl("/chapter/ch2"),
            title: "Chapter 2",
          },
        ],
      },
    ...
    await waitForTabStateCleared(
      context,
      tabId,
      "supported-to-unsupported-test"
    )
  })

  test("side panel shows unsupported state after navigating away from a supported page", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await expect(
      sidepanel.getByRole("button", { name: /Select chapters/i })
    ).toBeVisible()
