...
async function switchSelectorToFlatChapterView(
  sidepanelPage: import("@playwright/test").Page
): Promise<void> {
  const showAllChaptersButton = sidepanelPage.getByRole("radio", {
    name: /All chapters/i,
  })
...
test.describe("selection consistency across tabs", () => {
  test("selection actions stay local to the side panel without creating session selection keys or mutating tab state", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await expect(spA.getByText("Hunter x Hunter")).toBeVisible({
      timeout: 15000,
    })
    ...
    await spA
      .locator('[data-testid="inline-item"][data-kind="standalone"]')
      .first()
      .click()

    await expect(
      spA.getByRole("button", { name: /Download \(1\)/i })
    ).toBeVisible()
...
test.describe("chapter selection flow", () => {
  test("expands inline selector and collapses after starting a download", async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test("does not persist chapter selections after closing and reopening the side panel", async ({
    ...
    await waitForTabStateById(page, context, tabId, (state) => {
      return (
        state.mangaId === "selection-reset-series" &&
        state.chapters?.length === baseChapters.length
      )
    })
    ...
    await expect(
      reopened.getByRole("button", { name: /Select Chapters/i })
    ).toBeVisible({ timeout: 15000 })
    await reopened.getByRole("button", { name: /Select Chapters/i }).click()

    await expect(
      reopened.getByRole("button", { name: /Download \(1\)/i })
    ).toHaveCount(0)
    await expect
      .poll(async () => {
        const activeTabState = await getSessionState<{
  ...
  test("does not show the deferred New quick-select control in the selector", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await waitForTabStateById(page, context, tabId, (state) => {
      return (
        state.mangaId === "no-new-quick-select" &&
        state.chapters?.length === baseChapters.length
      )
    })
  ...
  test("locked chapters are displayed but cannot be selected", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    const baseChapters = [
      {
        id: "locked-chapter-1",
        url: lockedChapterUrl,
        title: "Locked Chapter 1",
        locked: true,
      },
      {
        id: "open-chapter-1",
        url: buildExampleUrl("/open-chapter-1"),
    ...
    const tabState = await waitForTabStateById(
      ...
      (state) => {
        return (
          ...
          state.chapters.some(
            (chapter) =>
              chapter.url === lockedChapterUrl && chapter.locked === true
          )
        )
      }
    )
    const lockedChapter = tabState.chapters.find(
      (chapter) => chapter.locked === true
    )
    const openChapter = tabState.chapters.find(
      (chapter) => chapter.locked !== true
    )
    ...
    const sp = await openSidepanelHarness(context, extensionId, page)
    await expect(sp.locator("#root")).toBeVisible()
    await expect(sp.getByText("Locked Selection Series")).toBeVisible({
      timeout: 15000,
    })
    ...
    await expect(
      sp.getByRole("checkbox", { name: new RegExp(lockedChapter!.title, "i") })
    ).toBeDisabled()
    await expect(
      sp.getByRole("checkbox", { name: new RegExp(lockedChapter!.title, "i") })
    ).not.toBeChecked()
    ...
    await expect(
      sp.getByRole("checkbox", { name: new RegExp(openChapter!.title, "i") })
    ).toBeChecked()
