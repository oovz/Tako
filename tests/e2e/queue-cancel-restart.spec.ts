...
test.describe("Queue cancel and restart behavior", () => {
  ...
  test("per-task Cancel and overflow Remove actions update global queue state", async ({
    ...
    await waitForGlobalState(
      context,
      (state) =>
        !state.downloadQueue.some((task) => task.id === "completed-task")
    )
  })

  test("inline cancellation confirmation keeps task downloading when user selects No", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await waitForGlobalState(context, (state) => {
      const t = state.downloadQueue.find(
        (task) => task.id === "active-task-no-confirm"
      )
  ...
  test("retry failed chapters creates a new queued task and marks original as retried", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    await waitForGlobalState(context, (state) => {
      const original = state.downloadQueue.find(
        (t) => t.id === "failed-with-mix"
      )
      if (!original || !original.isRetried) return false

      const retryTasks = state.downloadQueue.filter(
        (t) => t.id !== "failed-with-mix" && t.mangaId === original.mangaId
      )
    ...
    await expect(sp.getByRole("button", { name: "Retry failed" })).toHaveCount(
      0
    )
  ...
  test("retry action includes both failed and partial_success chapters", async ({
    context,
    extensionId,
    page,
  }) => {
    ...
    const tasks: DownloadTaskState[] = [
      makeTask({
        ...
        chapters: [
          ...
          makeChapter(
            buildExampleUrl("/rp3"),
            "partial_success" as ChapterState["status"]
          ),
        ],
      }),
    ]
    ...
    await waitForGlobalState(context, (state) => {
      ...
      const retryTask = state.downloadQueue.find(
        (task) =>
          task.id !== "retry-partial" && task.mangaId === original.mangaId
      )
      if (!retryTask) {
        return false
      }
