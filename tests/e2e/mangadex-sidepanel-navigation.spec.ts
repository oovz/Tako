...
test.describe("MangaDex side panel navigation workflows (mocked)", () => {
  ...
  test("series -> front page -> series (new) reinitializes to new series state", async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test("SPA: series -> front page -> series (new) via history.pushState requires refresh before new tab state appears", async ({
    context,
    extensionId,
    page,
  }) => {
