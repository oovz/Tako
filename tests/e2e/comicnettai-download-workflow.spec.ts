...
test.describe("Comic Nettai download workflow (mocked)", () => {
  ...
  test("completes a single-chapter download through the PUBLUS configuration pipeline", async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test("completes a CBZ through the default browser-download handoff", async ({
    context,
    extensionId,
    page,
  }) => {
    const series = ComicNettai.BASIC_SERIES.series
    await page.goto(`${COMICNETTAI_BASE_URL}/book/${series.seriesId}`, {
    ...
    if (!firstChapter) {
      throw new Error(`No downloadable chapter found for ${series.seriesId}`)
    }
