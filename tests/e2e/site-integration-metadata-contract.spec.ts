...
test.describe("Site integration chapter metadata contracts (mocked)", () => {
  test("MangaDex keeps chapter titles and API volume fields separate", async ({
    context,
    page,
  }) => {
    const series = Mangadex.BASIC_SERIES.series
    await page.goto(
      `${MANGADEX_BASE_URL}/title/${series.seriesId}/hunter-x-hunter`,
    ...
    const firstChapter = state.chapters.find(
      (chapter) => chapter.id === "afaebc64-83df-4f11-b2b0-5ef4fcc8144c"
    )
  ...
  test("Pixiv Comic composes numbering and subtitle once without synthetic volumes", async ({
    context,
    page,
  }) => {
    const series = PixivComic.BASIC_SERIES.series
    await page.goto(`${PIXIV_COMIC_BASE_URL}/works/${series.seriesId}`, {
      waitUntil: "domcontentloaded",
    })
    const tabId = await getTabId(page, context)
  ...
  test("Shonen Jump+ uses pagination API episode titles without volume metadata", async ({
    context,
    page,
  }) => {
    const series = ShonenJumpPlus.BASIC_SERIES.series
    await page.goto(`${SHONENJUMPPLUS_BASE_URL}/episode/${series.seriesId}`, {
      waitUntil: "domcontentloaded",
    })
    const tabId = await getTabId(page, context)
    ...
    const firstEpisode = state.chapters.find(
      (chapter) => chapter.id === series.seriesId
    )
  ...
  test("Manhuagui preserves category headings as volumes and anchor titles as chapter titles", async ({
    context,
    page,
  }) => {
    const series = Manhuagui.KIMETSU_SERIES.series
    await page.goto(`${MANHUAGUI_BASE_URL}/comic/${series.seriesId}/`, {
      waitUntil: "domcontentloaded",
    })
    const tabId = await getTabId(page, context)
