...
- **側邊欄命令中心**
  — 章節選擇、佇列和進度都在 Chrome 側邊欄中，就在你閱讀的頁面旁邊。無需額外分頁，無需反覆儲存對話框。
- **真正的佇列與重試**
  — 排隊數十個章節，查看每張圖片的進度，自動或手動重試失敗的下載。同一時間只處理一個任務，保持穩定。
- **乾淨的匯出**
  — 預設透過 Chrome 下載儲存 CBZ 檔案。也可以選擇 ZIP 或散裝圖片資料夾。自訂路徑和檔名範本相容 Komga、Kavita、Calibre 等庫管理工具。
- **優化的網站整合**
...
- **檔案系統存取支援**
  ...
  Downloads 或取消的選項。
- **ComicInfo.xml 產生**
  — 在 CBZ 封存中嵌入系列詮釋資料、章節編號、作者等資訊，相容漫畫庫管理器。
- **隱私優先**
