...
- **侧边栏命令中心**
  — 章节选择、队列和进度都在 Chrome 侧边栏中，就在你阅读的页面旁边。无需额外标签页，无需反复保存对话框。
- **真正的队列与重试**
  — 排队数十个章节，查看每张图片的进度，自动或手动重试失败的下载。同一时间只处理一个任务，保持稳定。
- **干净的导出**
  — 默认通过 Chrome 下载保存 CBZ 文件。也可以选择 ZIP 或散装图片文件夹。自定义路径和文件名模板兼容 Komga、Kavita、Calibre 等库管理工具。
- **优化的站点集成**
...
- **文件系统访问支持**
  ...
  Downloads 或取消的选项。
- **ComicInfo.xml 生成**
  — 在 CBZ 归档中嵌入系列元数据、章节编号、作者等信息，兼容漫画库管理器。
- **隐私优先**
