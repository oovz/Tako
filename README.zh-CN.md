...
[安装](#开始使用) · [功能](#功能) · [支持的网站](#支持的网站) ·
[Wiki](https://github.com/oovz/Tako/wiki) · [隐私](#隐私)
...
- **侧边栏命令中心**
  — 章节选择、队列和下载进度直接在 Chrome 侧边栏中操作，就在你阅读的页面旁边；完整历史与详细设置可通过选项页面管理。
- **真正的队列与重试**
  — 排队数十个章节，查看每张图片的进度，自动或手动重试失败的下载。同一时间只处理一个任务，保持稳定。
- **干净的导出**
  — 默认通过 Chrome 下载保存 CBZ 文件。也可以选择 ZIP 或散装图片文件夹。自定义路径和文件名模板可适配 Komga、Kavita 等库管理工具以及 Calibre 等阅读器。
- **优化的站点集成**
  — 每个支持的网站都有针对性的页面结构、图片 CDN 和元数据处理 — 而非通用抓取。
- **统一设置页面**
  — 输出格式、模板、速率限制和重试的全局默认值与站点覆盖都在选项页面中。
- **文件系统访问支持**
  — 选择自定义下载文件夹，Tako 直接写入。若访问失效，Tako 会停止该任务，并提供重新授权、选择其他文件夹、明确改用 Chrome
  Downloads 或取消的选项。
- **ComicInfo.xml 生成**
  — 在 CBZ 归档中嵌入系列元数据、章节编号、作者等信息，兼容 Komga 与 Kavita 等漫画库管理器。
- **隐私优先**
  — 不向开发者发送分析或遥测。所需请求直接发送给已启用的提供方；启用 MangaDex 时，MangaDex@Home 的报告会明确说明。

## 支持的网站

| 网站                                        |                状态                 |
| ------------------------------------------- | :---------------------------------: |
| [MangaDex](https://mangadex.org)            | ✅（默认关闭；需要可选 HTTPS 权限） |
...
Shonen Jump+ 需从包含数字 ID 的 `/episode/{id}` 页面使用；主页和 `/series`
目录路由有意不受支持。
...
1. 从
   [Chrome 应用商店](https://chromewebstore.google.com/detail/tako-manga-downloader/hlodmckfkmbenkknmailfekehgajpmbb)安装。
2. 打开一个受支持的漫画系列页面。
3. 点击 Tako 图标打开侧边栏。
4. 选择章节，点击**下载**，查看队列。
...
Tako 当前针对 Chrome 150 或更高版本。本地构建需要 Node.js
...
然后打开
`chrome://extensions`，启用**开发者模式**，选择**加载已解压的扩展程序**，选中
`.output\chrome-mv3`。
</details>
...
Tako 将设置、队列状态和历史记录存储在本地浏览器中。网络请求直接发送到受支持的网站及下载所需的相关基础设施。没有由开发者运营的分析或遥测服务。启用 MangaDex 后，所需的 MangaDex@Home 报告会直接发送给 MangaDex。
