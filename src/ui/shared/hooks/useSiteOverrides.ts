...
export function useSiteOverrides() {
  const { value: overrides, hydrated } =
    useChromeStorageValue<SiteOverridesMap>({
      areaName: "local",
  ...
  const updateSiteOverride = useCallback(
    async (siteId: string, updates: SiteOverrideRecord) => {
