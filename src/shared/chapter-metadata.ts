...
function mapContentRatingToAgeRating(
  contentRating: string | undefined
): ComicInfoV2["AgeRating"] | undefined {
...
function mapReadingDirectionToManga(
  readingDirection: string | undefined
): ComicInfoV2["Manga"] | undefined {
  if (!readingDirection) {
    return undefined
  }
...
export function buildSeriesComicInfoBase(
  seriesTitle: string,
  metadata?: SeriesMetadataLike
): ComicInfoV2 {
  const base: ComicInfoV2 = {
    ...
    Genre:
      Array.isArray(metadata?.genres) && metadata.genres.length > 0
        ...
        : undefined,
    CommunityRating:
      typeof metadata?.communityRating === "number"
...
export function composeChapterMetadata(
  book: Book,
  input: ChapterInput
): Chapter {
