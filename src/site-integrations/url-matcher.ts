...
export interface MatchUrlOptions {
  /**
   * Match a shipped integration's declared URL pattern even when the user has
   * it disabled. Callers that only need to validate a URL's known shape (for
   * example, a stale-message guard) can use this without treating the
   * integration as operationally enabled.
   */
...
export function matchUrl(
  ...
  for (const candidate of candidates) {
    if (!options.includeDisabled && !isEnabled(candidate.integrationId)) {
      continue
    }
    ...
    if (testPatterns(pathname, candidate.seriesRegex)) {
      ...
      if (
        candidate.excludeRegex.length > 0 &&
        testPatterns(pathname, candidate.excludeRegex)
      ) {
