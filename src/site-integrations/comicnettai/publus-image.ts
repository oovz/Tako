...
class PublusRandom {
  ...
  private transform: (
    ...
    right: number,
    tail: number
  ) => number = PUBLUS_RANDOM_FUNCTIONS[0]
  ...
  select(paramIndex: number, functionIndex: number): void {
    ...
    this.transform =
      PUBLUS_RANDOM_FUNCTIONS[functionIndex] ?? PUBLUS_RANDOM_FUNCTIONS[0]
  }
    ...
    if (selected < remainingColumns) {
      if (selected < horizontalPivot) {
        ...
        for (
          ;
          start > 0 &&
          !(leftColumn + remainingColumns <= horizontal[start - 1]);
          start -= 1
        );
        let end = topRow + remainingRows
        for (
          ;
          end < rows && !(leftColumn + remainingColumns <= horizontal[end]);
          end += 1
        );
      ...
      if (selected - remainingColumns < verticalPivot) {
        ...
        for (
          ;
          start > 0 && !(topRow + remainingRows <= vertical[start - 1]);
          start -= 1
        );
        let end = leftColumn + remainingColumns
        for (
          ;
          end < columns && !(topRow + remainingRows <= vertical[end]);
          end += 1
        );
...
function buildPublusPermutation(
  seed: number,
  columnSeed: number,
  rowSeed: number,
  modeSeed: number
  ...
  const paramIndex =
    Math.floor(((combinedHigh >>> 16) - functionIndex) / functionCount) %
    modeCount
  ...
  const edgeParamIndex =
    Math.floor((edgeSelector - edgeFunctionIndex) / functionCount) % modeCount
  ...
  for (let column = 0; column < columns; column += 1) {
    for (let row = 0; row < rows; row += 1) {
      ...
      const encodedColumn =
        column < horizontalCuts[row] ? column : column + columnWrap
      const encodedRow = row < verticalCuts[column] ? row : row + rowWrap
      const targetColumn =
        sourceColumn < bottomVerticalCuts[sourceRow]
          ? sourceColumn
          : sourceColumn + columnWrap
      const targetRow =
        sourceRow < rightHorizontalCuts[sourceColumn]
          ? sourceRow
          : sourceRow + rowWrap
  ...
  for (let column = 0; column < columns; column += 1) {
    ...
    const targetColumn =
      shuffledColumn < rightEdgeColumn
        ? shuffledColumn
        : shuffledColumn + columnWrap
    output.push(
      rightHorizontalCuts[shuffledColumn] * encodedColumns + encodedColumn
    )
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    const targetRow =
      shuffledRow < bottomEdgeRow ? shuffledRow : shuffledRow + rowWrap
...
function appendTileRects(
  ...
  while (index < end) {
    ...
    const sourceColumn = Math.floor(
      (encodedX - targetRow) / dimensions.encodedRows
    )
    const sourceRow = Math.floor(
      (encodedY - targetColumn) / dimensions.encodedColumns
    )

    output.push({
      srcX:
        targetColumn * dimensions.tileWidth -
        (targetColumn > dimensions.columns
          ? dimensions.overflowWidthOffset
          : 0),
      srcY:
        targetRow * dimensions.tileHeight -
        (targetRow > dimensions.rows ? dimensions.overflowHeightOffset : 0),
      destX:
        sourceColumn * dimensions.tileWidth -
        (sourceColumn > dimensions.columns
          ? dimensions.overflowWidthOffset
          : 0),
      destY:
        sourceRow * dimensions.tileHeight -
        (sourceRow > dimensions.rows ? dimensions.overflowHeightOffset : 0),
      width,
      height,
    })
  }
...
export function buildPublusPageTileRects(
  input: PublusPageTileRectInput
): PublusTileRect[] {
  ...
  const paramIndex =
    Math.floor((modeSelector - functionIndex) / PUBLUS_FUNCTION_COUNT) %
    PUBLUS_PARAM_COUNT
  const random = new PublusRandom()
  random.select(paramIndex, functionIndex)
  ...
  const permutationSeed =
    random.next(65536) +
    random.next(65536) * 65536 +
    random.next(512) * 4294967296
  ...
  const pairs = buildPublusPermutation(
    permutationSeed,
    columnSeed,
    rowSeed,
    modeSeed
...
export function appendPublusImageMetadata(
  imageUrl: string,
...
export function parsePublusImageTransportUrl(imageUrl: string): {
  sourceUrl: string
  metadata: PublusImageMetadata | null
} {
  const url = new URL(imageUrl)
  const fragment = new URLSearchParams(
    url.hash.startsWith("#") ? url.hash.slice(1) : url.hash
  ...
  const decoded = JSON.parse(
    new TextDecoder().decode(base64UrlToBytes(encodedMetadata))
  ) as unknown
  return {
