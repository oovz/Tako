import { assertSafePublicHttpsUrl } from "../request-policy"
...
function buildMangadexImageUrl(
  baseUrl: string,
  quality: "data" | "data-saver",
  chapterHash: string,
  filename: string
): string {
  const trustedBaseUrl = assertSafePublicHttpsUrl(
    baseUrl,
    "MangaDex image server"
  )
    .toString()
    .replace(/\/$/, "")
  return `${trustedBaseUrl}/${quality}/${chapterHash}/${filename}`
}

export function parseMangadexImageDeliveryTarget(
  imageUrl: string
): MangadexImageDeliveryTarget | null {
  const parsedUrl = assertSafePublicHttpsUrl(imageUrl, "MangaDex image request")
  const pathnameParts = parsedUrl.pathname.split("/").filter(Boolean)
  const qualityIndex = pathnameParts.findIndex(
    (part) => part === "data" || part === "data-saver"
  )
  ...
  const baseUrl =
    basePath.length > 0 ? `${parsedUrl.origin}/${basePath}` : parsedUrl.origin
...
function resolveMangadexImageFilenameForQuality(
  files: string[],
  ...
  const prefixMatch =
    pagePrefix.length > 0
      ? files.find(
          (file) =>
            file.startsWith(`${pagePrefix}-`) ||
            file === deliveryTarget.filename
        )
      : undefined
...
export function resolveMangadexImageUrlForQuality(
  ...
  const files =
    quality === "data" ? atHome.chapter.data : atHome.chapter.dataSaver
  const resolvedFilename = resolveMangadexImageFilenameForQuality(
    files,
    deliveryTarget
  )

  return buildMangadexImageUrl(
    atHome.baseUrl,
    quality,
    atHome.chapter.hash,
    resolvedFilename
...
export function buildPageUrls(
  ...
  const baseUrl = assertSafePublicHttpsUrl(
    atHome.baseUrl,
    "MangaDex@Home image server"
  )
    .toString()
    .replace(/\/$/, "")
  ...
  if (!Array.isArray(files)) {
    throw new Error(
      "Malformed MangaDex at-home response: missing image file list"
    )
  }
  ...
  if (!files.every((f): f is string => typeof f === "string" && f.length > 0)) {
    throw new Error(
      "Malformed MangaDex at-home response: file list contains non-string or empty entries"
    )
  }
