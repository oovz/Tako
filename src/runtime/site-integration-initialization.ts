 ...
 * implementations are statically registered from separate background/offscreen
 * modules so bundlers do not pull wrong-context code into another
...
let integrationEnablementListenerRegistered = false

/**
 * Default enablement loader: reads `chrome.storage.local` directly.
 *
 * Valid in the background service worker, which has access to `chrome.storage`.
 * MUST NOT be used in the offscreen document,
...
export async function initializeSiteIntegrationEnablement(
  ...
  integrationEnablementInitPromise = (async () => {
    ...
    // (background). The offscreen document only exposes chrome.runtime,
    // so it must not register a storage change listener here.
    if (
      !integrationEnablementListenerRegistered &&
      ...
      integrationEnablementListenerRegistered = true
    }
...
export function registerSiteIntegrationRuntime(
  integration: RuntimeSiteIntegration
): void {
...
async function registerSiteIntegrationMetadata(): Promise<void> {
  ...
  logger.debug(
    "📋 Registering site integration metadata for background context..."
  )
