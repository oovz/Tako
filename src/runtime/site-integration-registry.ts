...
class SiteIntegrationRegistry {
  ...
  register(info: SiteIntegrationInfo): void {
    ...
    const incomingAddsBackground =
      !!incomingIntegration?.background && !existingHasBackground
    const incomingAddsOffscreen =
      !!incomingIntegration?.offscreen && !existingHasOffscreen

    // Skip if the incoming registration adds no new runtime surface.
    if (
      existing &&
      incomingIntegration &&
      !incomingAddsBackground &&
      !incomingAddsOffscreen
    ) {
      logger.debug(
        `⏭️ Site integration ${info.id} already registered, skipping`
      )
      return
    }
    ...
    if (existing && !incomingIntegration) {
      logger.debug(
        `⏭️ Site integration metadata ${info.id} already registered, skipping`
      )
      return
    }
    ...
    const mergedInfo: SiteIntegrationInfo =
      existing && incomingIntegration
        ? {
            ...existing,
            ...info,
            integration: {
              ...
              background:
                incomingIntegration.background ??
                existing.integration?.background,
              offscreen:
                incomingIntegration.offscreen ??
                existing.integration?.offscreen,
            },
          }
        : info
  ...
  private validateIntegration(info: SiteIntegrationInfo): void {
    ...
    for (const field of info.customSettings ?? []) {
      assertValidSettingsFieldSchema(field)
    }
    // Skip validation if it's metadata-only registration
    if (!info.integration) {
      logger.debug(
        `📋 Registering metadata-only for site integration: ${info.id}`
      )
      return
    }
    // Validate full site integration if present
    if (!info.integration.background && !info.integration.offscreen) {
      throw new Error(
        "Site integration must have background or offscreen implementation"
      )
    }
 ...
 */
export function findSiteIntegrationForUrl(
  url: string
): SiteIntegrationInfo | null {
